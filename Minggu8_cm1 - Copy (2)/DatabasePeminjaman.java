public class DatabasePeminjaman {
    ...
    public void sortByName() {
        ...
        for (int i = 0; i < dataMhs.length - 1; i++) {
            ...
            for (int j = i + 1; j < dataMhs.length; j++) {
                // asc
                int compare = dataMhs[idx].nama.compareTo(dataMhs[j].nama);
                if (compare > 0) {
                    idx = j;
                }
            }
    ...
    // modifikasi B3
    public void searchByName(int start, int end, String keyword) {
        // sorting asc
        sortByName();

        if (end >= start) {
            int mean = (start + end) / 2;
            int compare = dataMhs[mean].nama.compareToIgnoreCase(keyword);
            if (compare == 0) {
                dataMhs[mean].tampilMahasiswa();
            } else if (compare < 0) {
                searchByName(mean + 1, end, keyword);
            } else {
                searchByName(start, mean - 1, keyword);
            }
        } else {
            System.out.println("data tidak ditemukan!");
        }
    ...
    //modifikasi C3
    public void avgLamaPeminjaman() {
        for (int i = 0; i < dataMhs.length; i++) {
            int sum = 0;
            int banyakPinjam = 0;
            for (int j = 0; j < dataPinjam.length; j++) {
                if (dataMhs[i].nim.equals(dataPinjam[j].mhs.nim)) {
                    sum += dataPinjam[j].lamaPinjam;
                    banyakPinjam++;
                }
            }

            // System.out.println(sum);
            // System.out.println(banyakPinjam);
            System.out.println("Nama : " + dataPinjam[i].mhs.nama + " | Rata - rata lama pinjam : " + (double) (sum / banyakPinjam));
        }
    }
}
